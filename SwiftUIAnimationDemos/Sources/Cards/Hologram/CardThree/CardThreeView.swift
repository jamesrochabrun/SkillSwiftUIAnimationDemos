    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            ...
            let effectiveTilt = CGPoint(
                x: motionManager.tilt.x + (isDragging ? dragOffset.width / 100 : 0),
                y: motionManager.tilt.y + (isDragging ? dragOffset.height / 100 : 0)
            )
            ...
            .rotation3DEffect(
                .degrees(effectiveTilt.x * 15),
                axis: (x: 0, y: 1, z: 0),
                perspective: 0.5
            )
            .rotation3DEffect(
                .degrees(-effectiveTilt.y * 15),
                axis: (x: 1, y: 0, z: 0),
                perspective: 0.5
            )
            .shadow(
